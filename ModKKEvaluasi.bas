...
' Mapping field → baris di sheet KK Evaluasi (kolom C/D/E per peserta)
' Sesuaikan nomor baris dengan sheet aktual
' Baris sesuai sheet "3. KK Evaluasi Kualifikasi" v1.4
Private Const ROW_NIB_NOMOR As Integer = 8
Private Const ROW_SS_NOMOR As Integer = 10
Private Const ROW_SS_TERVERIFIKASI As Integer = 11
...
Private Const ROW_SKP_CATATAN As Integer = 31
...
Public Sub MuatKKEvaluasi()
    ...
    ' 1. Baca kode_tender dari sheet "1. Input Data" cell E5
    ...
    ' Bersihkan dulu kolom C/D/E untuk baris data (baris 8-54)
    ...
    ' Isi data per peserta
    Dim i As Integer
    For i = 1 To itemCount
        If i > 3 Then Exit For  ' maks 3 peserta
        Dim it As Variant
        it = items(i - 1)
        Dim col As Integer
        col = colMap(i)

        wsKK.Cells(ROW_NIB_NOMOR, col).Value = GetF(it, "nib_nomor")
        wsKK.Cells(ROW_SS_NOMOR, col).Value = GetF(it, "ss_nomor")
        wsKK.Cells(ROW_SS_TERVERIFIKASI, col).Value = GetF(it, "ss_terverifikasi")
        wsKK.Cells(ROW_SBU_NOMOR, col).Value = GetF(it, "sbu_nomor")
        wsKK.Cells(ROW_SBU_BERLAKU, col).Value = GetF(it, "sbu_berlaku")
        wsKK.Cells(ROW_SBU_KUALIFIKASI, col).Value = GetF(it, "sbu_kualifikasi")
        wsKK.Cells(ROW_SBU_SUBKLAS, col).Value = GetF(it, "sbu_subklas_label")
        wsKK.Cells(ROW_PGL1_NAMA, col).Value = GetF(it, "pgl1_nama")
        ...
        ' Baris 31: jumlah pekerjaan berjalan, baris 33: SKP
        wsKK.Cells(ROW_SKP_CATATAN, col).Value = GetF(it, "skp_catatan")
        wsKK.Cells(ROW_SKP, col).Value = GetF(it, "skp")
        wsKK.Cells(ROW_NPWP, col).Value = GetF(it, "npwp")
        Dim kswpVal As String: kswpVal = GetF(it, "kswp_status")
        If GetF(it, "skp_berbeda") = "true" Then
            wsKK.Cells(ROW_SKP_CATATAN, col).Value = "[!] BERBEDA dg Formulir — " & GetF(it, "skp_catatan")
        End If
        wsKK.Cells(ROW_KSWP, col).Value = kswpVal
        wsKK.Cells(ROW_AKTA_P_NOMOR, col).Value = GetF(it, "akta_p_nomor")
        ...
        wsKK.Cells(ROW_AKTA_K_NOMOR, col).Value = GetF(it, "akta_k_nomor")
        ...
        wsKK.Cells(ROW_KINERJA_NILAI, col).Value = GetF(it, "kinerja_nilai")
        wsKK.Cells(ROW_KINERJA_KATEGORI, col).Value = GetF(it, "kinerja_kategori")

        ' MS/TMS: TIDAK VALID atau tidak ada NIB → TMS, else MS
        Dim msVal As String
        If UCase(GetF(it, "kswp_status")) = "TIDAK VALID" Or GetF(it, "nib_nomor") = "" Then
            msVal = "TMS"
        Else
            msVal = "MS"
        End If
...
' ============================================================
' PARSER: JSON array → array of Scripting.Dictionary
' ============================================================
Private Sub ParseKKJSON(json As String, ByRef items() As Variant, ByRef itemCount As Integer)
    ...
    Do
        ...
        ' Cari brace penutup yang sesuai (depth tracking)
...
Private Function GetF(fields As Variant, fieldName As String) As String
    ' Map nama field ke indeks array
