...
def format_desc(url: str, perubahan, pokja: str) -> str:
    try:    jml = int(float(str(perubahan).replace(',', '.')))
    except: jml = 0
...
def insert_events(service, df: pd.DataFrame, url: str, members: str):
    ...
    for _, row in df.iterrows():
        ...
        evt = {
            ...
            'description': format_desc(url, row['Perubahan'], members),
...
def sync_all():
    ...
    for idx, row in db.iterrows():
        ...
        n = insert_events(service, df_jadwal, url, members)
