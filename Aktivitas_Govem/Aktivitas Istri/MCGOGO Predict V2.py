...
# 1. UKURAN KOTAK SNAP (TETAP 250x35 SESUAI BASE CODE)
CROP_WIDTH = 250   
CROP_HEIGHT = 35   
...
class TrackerFinalFix:
    def __init__(self):
        self.root = tk.Tk()
        self.screen_w = self.root.winfo_screenwidth()
        self.screen_h = self.root.winfo_screenheight()

        print(f"--- TRACKER FINAL FIX ---")
        print(f"Klik Kiri / Numpad : Naik Pertemuan (Skip)")
        print(f"Klik Kanan         : Turun Pertemuan (Undo)")
        print(f"Double Klik Kanan  : Mati/Hidup")
        ...
        # --- MOUSE BINDING (SESUAI INSTRUKSI) ---
        self.canvas.bind("<Button-1>", lambda e: self.on_left_click(e))
        self.canvas.bind("<Button-3>", lambda e: self.on_right_click(e))
    ...
    def setup_ui_buttons(self):
        ...
        current_x += btn_w + gap
        self.create_btn(77, "-", current_x, start_y, "#222222", "white", hideable=False) # Hide Button
    ...
    def on_double_right_click(self, event):
        # DOUBLE RIGHT CLICK (MATIKAN PLAYER)
        bid = self.get_clicked_btn(event)
        if bid in self.players: self.handle_kill(bid)

    # --- LOGIC UTAMA ---
    def handle_left(self, bid):
        # KLIK KIRI / NUMPAD: SELALU NAIKKAN URUTAN (SKIP)
        if bid == 0: self.reset_game()
            ...
            # Logic: Selalu tambah counter, meskipun sudah ada nomor (Untuk fitur Skip)
    ...
    def handle_kill(self, bid):
        ...
        if bid in self.players:
            p = self.players[bid]
            # Karena Double Click Kanan didahului Single Click Kanan (Undo),
            # maka player ini pasti sudah dalam keadaan Reset (Undo).
            # Kita tinggal ubah status visualnya saja ke Mati/Hidup.
